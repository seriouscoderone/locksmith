...
**Interfaces — Consumes:** `conn_table` (Task 1). **Produces:** `$connect`→200; `$disconnect`→idempotent `DeleteItem(connectionId)`; subscribe (via `$default`, action=`subscribe`)→ verify the embedded signed `qry` with `_hby.psr.parse(framed=True)` + extract `q.pre`/`q.topics` (reuse `_detect_mbx_query` logic, `mailbox_handler.py:369`), then `PutItem {connectionId, pre, topics, connectedAt, expireAt}`; invalid/unverifiable qry → register nothing + error frame.

- [ ] **Step 1 (test):** subscribe with a valid signed `qry r=/mbx` → a registry row keyed by `connectionId` with `pre`/`topics`/TTL; an unsigned/forged qry → no row + error. `$disconnect` → row deleted, idempotent (no error if already gone). Use `moto` + a temp Habery holding the sender's inception (`conftest` pattern). RED.
- [ ] **Step 2 (impl):** implement per **WS spec §5.3**. The subscribe handler needs the full Habery (heavy cold start — fine, runs only on subscribe). GREEN.
- [ ] **Step 3:** commit.
