...
from keri_serviceaid.providers import (
    admit_grant, frame_grant_for, issue_credential, revoke_credential,
)
...
class ServiceaidRevokeDoer(doing.DoDoer):
    """Revokes an already-issued credential through
    `keri_serviceaid.providers.revoke_credential` (local TEL rev + KEL anchor),
    then streams the updated TEL + issuer KEL to the holder over Locksmith's
    peer-aware transport so the holder's Tevery applies the `rev`.

    Mirrors `ServiceaidGrantDoer`'s delivery tail, but delivers a RAW TEL
    update, NOT an IPEX exn: `credentialing.sendArtifacts` already streams the
    issuer KEL (now carrying the revocation anchor) and the credential's full
    TEL (now carrying the `rev` at sn=1) -- exactly what a holder needs to
    observe the revocation. No `/ipex/*` exn is framed, so the holder gets no
    spurious "credential offer" note; the gate reacts to the observed TEL
    state (see PluginManager.recheck_gates / reevaluate_role_gates).

    Recipient is resolved from the credential itself (`creder.attrib["i"]`) --
    the issuer already knows who holds what it issued; callers pass only the
    credential SAID.
    """

    def __init__(self, app, *, credential_said, hab_pre, message: str = "", **kwa):
        self.app = app
        self.hby = app.vault.hby
        self.rgy = app.vault.rgy
        self.signal_bridge = app.vault.signals
        ...
        self.hab_pre = hab_pre
        self.message = message
        doers = [doing.doify(self.revokeDo)]
        super(ServiceaidRevokeDoer, self).__init__(doers=doers, **kwa)
    ...
    def revokeDo(self, tymth, tock=0.0, **opts):
        ...
        try:
            hab = self.hby.habs.get(self.hab_pre)
            if not hab:
                sink.on_event("RevokeCredentialDoer", "revoke_failed", {
                    "error": "Issuer identifier not found", "success": False,
                    "credential_said": self.credential_said})
                return
            ...
            if creder is None:
                sink.on_event("RevokeCredentialDoer", "revoke_failed", {
                    "error": f"Credential {self.credential_said} not found",
                    "success": False, "credential_said": self.credential_said})
                return
            recipient = creder.attrib.get("i")
            if not recipient:
                sink.on_event("RevokeCredentialDoer", "revoke_failed", {
                    "error": "Credential has no holder (attrib 'i') to notify",
                    "success": False, "credential_said": self.credential_said})
                return

            # Local revoke: flips the TEL to rev, anchors it, completes in-process.
            registry_name = _registry_name_for(self.rgy, self.credential_said)
            revoke_credential(
                self.hby, hab, self.rgy,
                credential_said=self.credential_said,
            ...
            # updated TEL (with the rev event).
            creder = self.rgy.reger.cloneCred(said=self.credential_said)[0]

            postman = PeerAwarePoster(
                hby=self.hby, hab=hab, recp=recipient,
                baser=self.app.vault.db, topic="credential")
            credentialing.sendArtifacts(
                self.hby, self.rgy.reger, postman, creder, recipient)
            ...
            channel = postman.last_outcome.value if postman.last_outcome else "mailbox"
            sink.on_event("RevokeCredentialDoer", "credential_revoked", {
                "success": True, "credential_said": self.credential_said,
                "recipient": recipient, "channel": channel})
            return
        except Exception as e:  # noqa: BLE001
            logger.exception(f"ServiceaidRevokeDoer failed: {e}")
            sink.on_event("RevokeCredentialDoer", "revoke_failed", {
...
def _registry_name_for(rgy, credential_said) -> str:
    """The human name of the registry a held credential belongs to.

    `revoke_credential` takes a registry NAME (as `issue_credential` does), but
    a revoker holds only the credential SAID; resolve the name from the
    credential's `regid` via the reger's name index (`rgy.reger.regs`, keyed by
    registry name -- see `keri.vdr.credentialing.Regery.loadRegistries`,
    credentialing.py:83, which resolves the same way:
    `for name, regord in self.reger.regs.getTopItemIter(): regk =
    regord.registryKey`). `getTopItemIter` returns the name as a 1-tuple
    (`Komer.getTopItemIter` reconstitutes the full keys tuple), so it is
    destructured to the bare string before returning. Raises if unresolved --
    revoking a credential whose registry we can't name is a real error, not a
    silent no-op.
    """
    regid = rgy.reger.cloneCred(said=credential_said)[0].regid
    for name, regord in rgy.reger.regs.getTopItemIter():
        if regord.registryKey == regid:
            return name[0]
    raise ValueError(f"no registry name found for credential {credential_said}")


def make_revoke_doer(app, hab, **kwargs):
    """Routing chokepoint for credential revocation: eligible habs (single-sig,
    unwitnessed) get `ServiceaidRevokeDoer`. There is NO legacy delivering
    revoke doer (the stock wallet's Revoke row action was a no-op stub), so an
    ineligible hab raises NotImplementedError -- mirroring `admit_grant`'s own
    GroupHab posture -- rather than silently doing nothing. The demo DOI is
    single-sig/witnessless, so the eligible branch is the live path.

    Accepts: `credential_said` (required), `hab_pre` (defaults to hab.pre),
    `message`.
    """
    if serviceaid_eligible(hab):
        return ServiceaidRevokeDoer(
            app,
            credential_said=kwargs["credential_said"],
            hab_pre=kwargs.get("hab_pre") or hab.pre,
            message=kwargs.get("message", ""),
    ...
    raise NotImplementedError(
        "revoke-over-serviceaid supports only single-sig, witnessless "
        "identifiers; GroupHab/witnessed revoke is not yet implemented")
