...
class LocksmithToolbar(QToolBar):
    ...
    def set_dock_active(self, locked: bool):
        """Update the dock button's icon pair to match the sidebar lock state.

        State is communicated entirely by the icon glyph swap (off rectangle
        vs filled rectangle). We deliberately do NOT call ``set_active`` —
        that helper paints a persistent hover-background that's visually
        indistinguishable from an actual hover, making the dock button look
        permanently stuck in hover state after a click.
        """
