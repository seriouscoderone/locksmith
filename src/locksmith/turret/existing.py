# -*- encoding: utf-8 -*-
"""
Existing for turret. SetupHby() has been modified slightly from the keripy implementation to better fit
the use-case.
"""
...
def setupHby(name, base="", bran=None, cf=None, temp=False):
    """ Create Habery off of existing directory, modified from keripy to adapt to turret functionality
    ...
    if aeid is None:
        print("Keystore must already exist")
        return None
    else:
        ks.close()
        try:
            ...
            raise kering.AuthError("Invalid Passcode")


@contextmanager
def existingHby(name, base="", bran=None):
    """
    Context manager wrapper for existing Habery instance.
    Will raise exception if Habery and database has not already been created.
